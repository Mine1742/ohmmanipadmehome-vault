...
[![Diagram for the storage architecture](https://microsoftlearning.github.io/Deploy-and-administer-Linux-virtual-machines-in-Azure/Instructions/Labs/media/lab03.png)](https://microsoftlearning.github.io/Deploy-and-administer-Linux-virtual-machines-in-Azure/Instructions/Labs/media/lab03.png)
...
3. Select the **Cloud Shell** icon in the top right of the Azure Portal.
    
    [![Screenshot of the Cloud Shell icon.](https://microsoftlearning.github.io/Deploy-and-administer-Linux-virtual-machines-in-Azure/Instructions/Labs/media/cloudshell-icon.png)](https://microsoftlearning.github.io/Deploy-and-administer-Linux-virtual-machines-in-Azure/Instructions/Labs/media/cloudshell-icon.png)
...
5. Select **No storage account required** and select your **Subscription**.
    
6. Click **Apply** and wait while a terminal is created.
    
7. This lab will use the Azure CLI so check out the documentation for virtual machines.
...
2. After the machine deploys, ensure the **powerState** is **VM running**. Make a note of the **publicIpAddress**. In a later step, you will need the public IP address and SSH key file to connect to the virtual machine.
    
3. Use [az vm disk](https://learn.microsoft.com/en-us/cli/azure/vm/disk?view=azure-cli-latest) to add a 4 GB data disk to the virtual machine.
...
4. Verify the data disk was created. This command will also show the operating system disk information.
...
1. To format and mount the data disk you will need to connect to the virtual machine. Substitute your public IP address. When prompted, confirm you want to connect. When successfully connected, the prompt will change to _adminuser@vm3_.
...
2. Retrieve information about the virtual machine storage. Make a note of the name of your new 4 GB disk. For example, **sdc**.
...
3. Use _parted_ to partition the disk. If necessary, change _sdc_ to your disk name.
...
4. Use _partprobe_ to inform the operating system of partition table changes. If necessary, change _sdc_ to your disk name.
...
5. Use _mkfs_ to build the Linux file system. If necessary, change _sdc_ to your disk name.
...
7. Mount the data disk to the directory.
...
8. Verify the disk now has a mount point.
...
9. Verify there are no files on the data disk.
...
10. Minimize the Cloud Shell, but don’t close the connection. In the next task, you will continue working in the virtual machine.
...
3. Select **+ Create** and on the **Basics** tab specify the following settings.
...
2. Select **+ File share** and on the **Basics** tab give the file share a name, `share1`.
    
3. On the **Backup** tab, ensure **Enable backup** is **not checked**.
    
    > You will configure backups in another lab.
    
4. Select **Review + create**, and then **Create**. Wait for the file share to deploy.
    
5. Select **Upload** and **Browse for files**.
    
6. Select a file and then click **Upload**.
    
    > If you have downloaded the lab files, there is a `filedoc.txt` file for you to use.
    
7. Select **Browse** left menu, to ensure your file was uploaded.
    
8. Select **Connect**.
    
    > The portal provides scripts to help connect from Windows, Linux, and macOS.
    
9. Switch to the **Linux** tab and select **Show script**.
    
10. **Copy to the clipboard** (icon bottom right in the script).
...
2. In the portal, locate your virtual machine (vm3).
    
3. In the **Security** settings, select **Identity**.
    
4. On the **System assigned** tab, toggle the **Status** to **On**.
...
5. **Save** your changes and select **Yes** to **Enable system assigned managed identity**.
...
2. Create a connection script using the vi editor. If you prefer you may use another editor.
...
3. Right click and **Paste as plain text**. Your connection script should display. Review the script.
    
4. Type **:wq** to write and quit. **Enter** to exit the editor.
...
6. Verify you are able to access the file share. The file you uploaded should be listed.
...
7. Copy the file share file to the virtual machine data disk.
...
8. Verify the file share was copied to the data disk.
...
9. Minimize the Cloud Shell, but don’t close the connection. In the next task, you will continue working on the virtual machine.
...
4. Select your **data** container and then click **Upload**.
    
5. **Browse for files**. Select a file then click **Upload**.
    
    > If you have downloaded the lab files, there is a `blobimage.png` file for you to use.
    
6. Select your uploaded file and copy the **URL**.
...
2. In the **Security** settings, select **Identity**.
...
4. Select **Add role assignment (Preview)** and provide the managed identity access to the storage account file share. Both read and write capability is required. **Save** your changes.
...
2. Download AzCopy and extract the file.
...
3. Make folder for azcopy and copy the executable to that directory.
...
4. Run this command to login azcopy as the virtual machine managed identity. Ensure there is a **succeeded** response.
...
5. Copy from Azure blob storage to the new virtual machine data drive. Substitute the blob file URL you copied from the portal.
...
6. Verify the file you uploaded to blob storage has been copied to the /datadrive.
