...
- No "retry enrichment" action for a card already sitting in the review list — a field that stayed unfilled before you added eBay credentials won't retroactively fill itself in on that same card; type the correct value in yourself and it teaches the checklist table for future cards. Check the server terminal for `[ebay_api]`/`[enrich]` log lines if a field or price estimate keeps falling back to web search after adding credentials — every failure now logs why
- **Real eBay sold-comp price data is currently unavailable, confirmed 2026-08-07 — not just hard to get.** Marketplace Insights (the only API that provides this) returned "Insufficient permissions," and eBay's own docs say it's "restricted and not open to new users at this time." No working alternative exists either — the old Finding API's `findCompletedItems` was fully decommissioned Feb 2025, and Trading/Inventory API only expose a seller's own listings, not marketplace-wide sold data. Price estimation stays on the web-search fallback for now. Browse API (card details) is a separate, unrestricted API and works regardless
